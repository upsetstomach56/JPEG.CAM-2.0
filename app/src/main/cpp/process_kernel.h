    ...
    int* work_0, int* work_1, int* work_2, int* work_h, int* h_line, int scaleDenom)
{
    ...
    bool useHalation = (halation > 0);
    bool useFullBloom = (bloom == 2 || bloom == 4);
    ...
    else if (useHalation) h_alpha = (scaleDenom == 4) ? 197 : ((scaleDenom == 2) ? 221 : 240);
    int inv_h = 256 - h_alpha;

    if (!work_0 || !work_h) return;

    if (useBloom && useHalation) {
        // Hot path for full recipes: same math as the generic path, fewer per-pixel branches.
        for (int x = 0; x < width; x++) {
            long long s0 = 0, sh = 0;
            int idx = x * 3;
            for (int y = 0; y <= 20; y++) {
                int w = (y <= 10) ? (y + 1) : (21 - y);
                uint8_t* src = rows[y] + idx;
        ...
        for (int x = 0; x < width; x++) {
            int idx = x * 3;
            uint8_t* src = rows[10] + idx;
            uint8_t* dst = out_row + idx;
            int v0_o = src[0], v1_o = src[1], v2_o = src[2];
            ...
            if (is_yuv) {
                ...
                dst[0] = (uint8_t)CLAMP(y_res);
                dst[1] = (uint8_t)CLAMP(cb_res);
                dst[2] = (uint8_t)CLAMP(cr_res);
            } else {
                ...
                dst[0] = (uint8_t)CLAMP(r_res);
                dst[1] = (uint8_t)CLAMP(g_res);
                dst[2] = (uint8_t)CLAMP(b_res);
            }
        }
    ...
    for (int x = 0; x < width; x++) {
        ...
        int blur_y = useBloom ? (work_0[x] / 121) : orig_y;
        int halation_y = useHalation ? (work_h[x] / 121) : 0;
    ...
    const uint8_t* externalGrainTexture = NULL) // <-- CHANGED from nullptr
{
    ...
    bool useAdvancedGrain = (advancedGrainExperimental == 1 && s_grain > 0);
    bool useTextureGrain = (advancedGrainExperimental == 2 && externalGrainTexture != NULL && grain > 0);
    bool useColorEffects = (s_chrome > 0 || s_blue > 0 || s_sat > 0);
    int textureMixBase = (grain >= 5) ? 256 : (grain * 51);
    int textureRowOffset = ((abs_y * scaleDenom) & 511) * 512 * 3;
    int textureX = 0;
    ...
    int nextRawY = prevRawY;
    if (width > 1) {
        const uint8_t* nextPixel = row + 3;
        nextRawY = (nextPixel[0] * 77 + nextPixel[1] * 150 + nextPixel[2] * 29) >> 8;
    }
    ...
    for (int x = 0, i = 0; x < width; x++, i += 3) {
        ...
        int y0Base = y0 * nativeLutSize;
        int y1Base = y1 * nativeLutSize;
        int z0Base = z0 * lutSize2;
        int z1Base = z1 * lutSize2;
        int v1, v2, w0, w1, w2, w3;

        if (dx >= dy_l) {
            if (dy_l >= dz) { v1=x1+y0Base+z0Base; v2=x1+y1Base+z0Base; w0=128-dx; w1=dx-dy_l; w2=dy_l-dz; w3=dz; }
            else if (dx >= dz) { v1=x1+y0Base+z0Base; v2=x1+y0Base+z1Base; w0=128-dx; w1=dx-dz; w2=dz-dy_l; w3=dy_l; }
            else { v1=x0+y0Base+z1Base; v2=x1+y0Base+z1Base; w0=128-dz; w1=dz-dx; w2=dx-dy_l; w3=dy_l; }
        } else {
            if (dz >= dy_l) { v1=x0+y0Base+z1Base; v2=x0+y1Base+z1Base; w0=128-dz; w1=dz-dy_l; w2=dy_l-dx; w3=dx; }
            else if (dz >= dx) { v1=x0+y1Base+z0Base; v2=x0+y1Base+z1Base; w0=128-dy_l; w1=dy_l-dz; w2=dz-dx; w3=dx; }
            else { v1=x0+y1Base+z0Base; v2=x1+y1Base+z0Base; w0=128-dy_l; w1=dy_l-dx; w2=dx-dz; w3=dz; }
        }

        const uint8_t* p = &nativeLut[(x0 + y0Base + z0Base)*3];
        const uint8_t* p1_v = &nativeLut[v1*3];
        const uint8_t* p2_v = &nativeLut[v2*3];
        const uint8_t* p3_v = &nativeLut[(x1 + y1Base + z1Base)*3];
        ...
        if (useTextureGrain) {
            ...
            if (env > 0) {
                // Fast 512x512 tiling
                int tex_idx = textureRowOffset + (textureX * 3);
        ...
        px[0] = (uint8_t)CLAMP(outR); px[1] = (uint8_t)CLAMP(outG); px[2] = (uint8_t)CLAMP(outB);

        prevRawY = currRawY;
        currRawY = nextRawY;
        if (x + 2 < width) {
            const uint8_t* nextPixel = px + 6;
            nextRawY = (nextPixel[0] * 77 + nextPixel[1] * 150 + nextPixel[2] * 29) >> 8;
        } else {
            nextRawY = currRawY;
        }
        textureX = (textureX + scaleDenom) & 511;
    }
}
    ...
    const uint8_t* externalGrainTexture = NULL) // <-- CHANGED from nullptr
{
    ...
    for(int x = 0; x < width; x++) {
        ...
        bool needsChroma = useColorEffects || halation > 0 || useTextureGrain || oldY != outY;
        int cb = 0, cr = 0;
        int sat = 0;
        if (needsChroma) {
            cb = row[i+1] - 128;
